...
#include "put_future_perf.hpp"

namespace chimera {

class OpFuture : public BasicFuture {
public:
    enum class Kind { None, Get, Put, PutPerf, Range };
...
public:
    OpFuture(ChimeraState& s, uint64_t id)
        : BasicFuture{s, id}, 
          ...
          put_f{s, id}, 
          range_f{s, id}, 
          put_perf_f{s, id} {} // Explicitly initialized to satisfy constructor binding
    ...
    void doPutPerf(uint64_t key, uint64_t val, PutPerfMode testing_mode, bool measuring = false) {
        kind = Kind::PutPerf;
        put_perf_f.doPutPerf(key, val, testing_mode, measuring);
    }
    ...
    bool isDone() const {
        switch (kind) {
            ...
            case Kind::PutPerf: return put_perf_f.isDone();
            case Kind::Range:   return range_f.isDone();
            case Kind::None:    return true;
            default:            return true; // Added default to satisfy -Wswitch-default
        }
    }
    ...
    bool isMeasuring() const {
        switch (kind) {
            ...
            case Kind::PutPerf: return put_perf_f.isMeasuring();
            case Kind::Range:   return range_f.isMeasuring();
            case Kind::None:    return false;
            default:            return false; // Added default to satisfy -Wswitch-default
        }
    }
    ...
    timepoint getStart() const {
        switch (kind) {
            ...
            case Kind::PutPerf: return put_perf_f.getStart();
            case Kind::Range:   return range_f.getStart();
            default:            return timepoint{}; // Added default to satisfy -Wswitch-default
        }
    }
    ...
    void addToOngoingRDMA(size_t server_idx, int64_t n) {
        switch (kind) {
            ...
            case Kind::PutPerf: put_perf_f.addToOngoingRDMA(server_idx, n); break;
            case Kind::Range:   range_f.addToOngoingRDMA(server_idx, n); break;
            case Kind::None:    break;
            default:            break; // Added default to satisfy -Wswitch-default
        }
    }
    ...
    bool tryStepForward() {
        switch (kind) {
            ...
            case Kind::PutPerf: {
                bool done = put_perf_f.tryStepForward();
                if (done) {
                    // TRICK THE PRINTER: Convert type identity back to legacy Put
                    // at the moment of completion. This forces reportStats() 
                    // to harvest the latency numbers into the PUT print buckets!
                    kind = Kind::Put; 
                }
                return done;
            }
